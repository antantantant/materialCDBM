...
Image = zeros(newRows, newCols); % we should import the blured one



red_patches = im2col(sample(:, :), [winsize winsize], 'sliding'); 
...
%%%% fill the image(at 128:130,128:130 with a random 3*3 patch from the
%%%% image
...
while nfilled < npixels    
    ...
    [pixelRows, pixelCols] = GetUnfilledNeighbors(filled, winsize);
     ...
     for i = 1:length(pixelRows)
       ...
       [bestMatches, SSD] = FindMatches(template, validMask, gaussMask, red_patches);
...
%% Get pixels at edge of synthesized texture
 function [pixelRows, pixelCols] = GetUnfilledNeighbors(filled, winsize) 
    ...
    [pixelRows, pixelCols] = find(border); % find: the index is not 0
    len = length(pixelRows); %how
     
     %randomly permute candidate pixels
     randIdx = randperm(len); %random generate len numbers to from 1:len
