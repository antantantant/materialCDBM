...
function [weight, params, history] = tirbm_train(xtr, params, Tlist)
    ...
    for i = 1:nbatch,
        ...
        for kcd = 1:params.kcd,
            ...
            if kcd == params.kcd,
                ...
                % hidden sampling
                hidprob = reshape(permute(hidprob, [2 1 3]), numtx, numhid*batchsize);
                [hidstate, ~] = sample_multinomial(hidprob, params.optgpu);
                hidstate = permute(reshape(hidstate(1:numtx, :), numtx, numhid, batchsize), [2 1 3]);
            end
        end
        ...
        if mod(i, 20) == 0,
            mean_err = mean(rec_err_epoch(i-19:i));
            mean_sparsity = mean(sparsity_epoch(i-19:i));
